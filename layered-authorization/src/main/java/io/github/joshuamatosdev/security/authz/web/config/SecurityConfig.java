...
import io.github.joshuamatosdev.security.authz.web.DocumentController;
...
/**
 * The coarse request gate (Layer 2, edge of this service) and method-security enablement. The gate
...
public class SecurityConfig {
    ...
    SecurityFilterChain securityFilterChain(final HttpSecurity http) throws Exception {
        http.authorizeHttpRequests(auth -> {
                for (final SecurityUrlGroup group : SecurityUrlGroup.values()) {
                    if (group.isPublic()) {
                        auth.requestMatchers(group.securityUrls()).permitAll();
                    }
                }
    ...
    UserDetailsService users() {
        final UserDetails member =
            User.withUsername("member").password("{noop}local-dev-only").roles("MEMBER").build();
        final UserDetails admin =
            User.withUsername("admin").password("{noop}local-dev-only").roles("PLATFORM_ADMIN").build();
