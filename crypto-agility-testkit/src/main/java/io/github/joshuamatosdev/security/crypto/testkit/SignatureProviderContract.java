...
public interface SignatureProviderContract {
    ...
    default byte[] contractPayload() {
        return "crypto-agility provider contract payload".getBytes(StandardCharsets.UTF_8);
    }
    ...
    default void tamperedPayloadsFail() {
        ...
        assertThat(provider.verify(
                        key.publicKey(),
                        "crypto-agility provider contract tampered payload".getBytes(StandardCharsets.UTF_8),
                        signature))
                .isFalse();
    }
