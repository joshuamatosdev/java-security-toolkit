...
public final class JcaSignatureProvider implements SignatureProvider {
    ...
    private final @Nullable ECParameterSpec expectedEcParameters;
    ...
    public boolean verify(final byte[] publicKey, final byte[] payload, final byte[] signature) {
        ...
        try {
            ...
            // The public key, payload, and signature are untrusted verification inputs. Anything
            // that cannot be parsed as a valid key under this algorithm is a verification failure,
            // never a thrown error: a relabeled forgery surfaces as InvalidKeySpecException
            // (GeneralSecurityException), while structurally invalid DER or a null/empty array make
            // the JCA decoders raise unchecked IllegalArgumentException/NullPointerException. Only a
            // genuinely missing algorithm (NoSuchAlgorithmException, caught above) is a config error.
            return false;
        }
        ...
        try {
            ...
            // A wrong, malformed, or null signature/payload is a verification failure, not a crash.
            return false;
        }
    }

    private static @Nullable ECParameterSpec expectedEcParameters(
            @Nullable final AlgorithmParameterSpec keyGenSpec) {
    ...
    private static void requireProviderConfigurationForAlgorithm(
            ...
            final String jcaSignatureName,
            final String keyFactoryName,
            @Nullable final ECParameterSpec expectedParameters) {
    ...
    private static boolean sameEcParameters(
            final ECParameterSpec expected, @Nullable final ECParameterSpec actual) {
        return actual != null
