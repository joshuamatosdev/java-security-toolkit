...
GRANT USAGE ON SCHEMA tenant_acme TO tenant_app;
GRANT USAGE ON SCHEMA tenant_globex TO tenant_app;

CREATE TABLE tenant_acme.document (
    id        id_v7 PRIMARY KEY,
    tenant_id uuid NOT NULL DEFAULT tenant_security.current_tenant_id(),
    title     text NOT NULL,
    body      text NOT NULL,
    CONSTRAINT document_tenant_check CHECK (tenant_id = '0190a000-0000-7000-8000-0000000000a1'::uuid)
);

ALTER TABLE tenant_acme.document ENABLE ROW LEVEL SECURITY;
ALTER TABLE tenant_acme.document FORCE ROW LEVEL SECURITY;

CREATE POLICY p_tenant_isolation ON tenant_acme.document
    FOR ALL TO public
    USING (
...
CREATE TABLE tenant_globex.document (
    id        id_v7 PRIMARY KEY,
    tenant_id uuid NOT NULL DEFAULT tenant_security.current_tenant_id(),
    title     text NOT NULL,
    body      text NOT NULL,
...
ALTER TABLE tenant_globex.document ENABLE ROW LEVEL SECURITY;
ALTER TABLE tenant_globex.document FORCE ROW LEVEL SECURITY;

CREATE POLICY p_tenant_isolation ON tenant_globex.document
    FOR ALL TO public
    USING (
        tenant_id = tenant_security.current_tenant_id()
    )
...
GRANT SELECT, DELETE ON tenant_acme.document TO tenant_app;
GRANT INSERT (title, body), UPDATE (title, body) ON tenant_acme.document TO tenant_app;
GRANT SELECT, DELETE ON tenant_globex.document TO tenant_app;
GRANT INSERT (title, body), UPDATE (title, body) ON tenant_globex.document TO tenant_app;
