...
-- application's one tenant-scoped table: note. Source of truth for the pattern:
...
--    self-settable session variable.
CREATE TABLE note (
...
CREATE POLICY p_tenant_isolation ON note
    FOR ALL TO public
    USING (
...
CREATE POLICY p_organization_scope ON note
    AS RESTRICTIVE
    FOR ALL TO public
    USING (
        pg_has_role(current_user, 'tenant_bypass', 'USAGE')
...
GRANT SELECT, DELETE ON note TO tenant_app;
GRANT INSERT (title, body), UPDATE (title, body) ON note TO tenant_app;
GRANT SELECT ON note TO tenant_bypass;
